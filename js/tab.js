...
import { updatePublicCounts, escapeHTML } from './utils.js';
...
function switchTab(tabId) {
    ...
    const isAdmin = !!(sess?.isAuthenticated && sess?.currentUser?.role === 'admin');
    const isJudge = !!(sess?.isAuthenticated && sess?.currentUser?.role === 'judge');
...
function updateTabsForRole() {
    const isAuth = state.auth?.isAuthenticated && state.auth?.currentUser;
    const role = isAuth ? state.auth.currentUser.role : 'public';
    ...
    Object.assign(tabs, rolePermissions[role] || {});

    const tabsContainer = document.querySelector('.tabs');
    if (!tabsContainer) return;
    tabsContainer.innerHTML = '';

    const tabOrder = ['public', 'teams', 'judges', 'standings', 'speakers', 'motions', 'results', 'draw', 'break', 'knockout', 'feedback', 'portal', 'import'];
    const tabNames = {
...
function renderStandings() {
    ...
    if (!state.standingsFilter) {
        state.standingsFilter = { status: 'all', search: '' };
    }
    ...
    ranked = applyStandingsFilters(ranked, state.standingsFilter, allRounds);

    let html = buildStandingsHeader(state.standingsFilter, catObj);
...
function buildStandingsHeader(filter, catObj = null) {
    const hasFilters = filter.status !== 'all' || filter.search;
    return `
        <div class="section">
            <div class="standings-header">
                <div style="display:flex;align-items:center;gap:10px;flex-wrap:wrap;">
                    <h2 class="u-mt-0" style="margin:0;">Team Standings</h2>
                    ${catObj ? `<span style="background:${catObj.color}18;border:1.5px solid ${catObj.color}50;color:${catObj.color};padding:3px 12px;border-radius:16px;font-size:13px;font-weight:700;">${catObj.icon} ${escapeHTML(catObj.name)}</span>
            ...
            <div class="standings-filter-bar">
                ...
                <div class="standings-filter-group standings-filter-group--flex">
                    <label class="standings-filter-label">Search</label>
                    <input type="text" id="search-filter" class="standings-filter-input"
                           value="${escapeHTML(filter.search)}"
                           oninput="window.updateStandingsFilter('search',this.value)"
...
function buildStandingsTable(ranked, allRounds) {
    ...
    ranked.forEach((team, index) => {
        ...
        html += `<tr>
            ...
            <td><span class="badge badge-mono">${escapeHTML(team.code || '')}</span></td>
        `;
...
function updateStandingsFilter(key, value) {
    if (!state.standingsFilter) state.standingsFilter = { status: 'all', search: '' };
    state.standingsFilter[key] = value;
    renderStandings();
}
...
function renderBreakDisplay() {
    ...
    const rows = breaking.map((t, i) => {
        ...
        return `<tr>
            ...
            <td><span class="badge badge-mono">${escapeHTML(t.code || '')}</span></td>
